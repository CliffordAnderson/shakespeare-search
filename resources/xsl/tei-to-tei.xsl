...
<xsl:stylesheet 
    ...
    -->
        <xsl:template match="/">
            <xsl:apply-templates xpath-default-namespace="http://www.tei-c.org/ns/1.0"/>
        </xsl:template>
        ...
        <xsl:template match="tei:front | tei:back | tei:encodingDesc | tei:w | tei:c | tei:lb | tei:pc | tei:fw "/>
    </xsl:stylesheet> 